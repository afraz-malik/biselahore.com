      ...
      >
        {activeSystems.map((system) => (
          <motion.a
            ...
            className="group flex items-start justify-between gap-3 rounded-2xl border border-border bg-card p-5 shadow-sm transition-colors hover:border-primary/40 hover:bg-accent/40"
          >
            <div className="flex flex-col gap-2">
              {system.featured ? (
                <Badge className="w-fit bg-primary text-primary-foreground">Featured</Badge>
              ) : null}
              <p className="text-base font-medium text-pretty">{system.title}</p>
            </div>
            <ArrowUpRight className="size-4.5 shrink-0 text-muted-foreground transition-colors group-hover:text-primary" />
          </motion.a>
        ))}
      </motion.div>
    </Section>
