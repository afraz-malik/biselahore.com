      ...
      >
        {activeSystems.map((system) => (
          <motion.a
            ...
            className="group relative flex items-start justify-between gap-3 overflow-hidden rounded-2xl border border-slate-300/85 bg-[linear-gradient(135deg,rgba(248,250,252,0.96)_0%,rgba(219,234,254,0.94)_48%,rgba(224,242,254,0.96)_100%)] p-5 text-slate-900 shadow-lg shadow-blue-950/10 transition-all duration-300 hover:-translate-y-1 hover:border-blue-500 hover:bg-[linear-gradient(135deg,rgba(59,130,246,0.98)_0%,rgba(37,99,235,0.97)_52%,rgba(14,165,233,0.96)_100%)] hover:text-white hover:shadow-xl hover:shadow-blue-500/20"
          >
            <div className="absolute inset-0 bg-[radial-gradient(circle_at_top_left,rgba(255,255,255,0.7),transparent_34%)] transition-opacity duration-300 group-hover:opacity-90" />
            <div className="relative flex flex-col gap-2">
              {system.featured ? (
                <Badge className="w-fit border border-blue-200/80 bg-white/70 text-blue-700 backdrop-blur group-hover:border-white/25 group-hover:bg-white/15 group-hover:text-white">Featured</Badge>
              ) : null}
              <p className="text-base font-semibold text-pretty">{system.title}</p>
            </div>
            <ArrowUpRight className="relative size-5 shrink-0 text-blue-700 transition-transform duration-300 group-hover:translate-x-0.5 group-hover:-translate-y-0.5 group-hover:text-white" />
          </motion.a>
        ))}
      </motion.div>
    </Section>
